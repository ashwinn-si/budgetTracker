...
- **Offline sync layer** lives in `lib/offline/syncQueue/` (split by concern: `expenses.ts`, `savings.ts`, `tags.ts`, `trips.ts`, `deleteLogs.ts`, `recovery.ts`, `flush.ts`, `pull/`). Always import from `@/lib/offline/syncQueue` (the `index.ts` barrel), and add new exports there. Queue writes with `sendOrQueue()` rather than re-implementing the online/offline branch.
- **Delete logs (Recycle Bin)** must stay one-row-per-entity under the id `del_${entityId}`: use `writeDeleteLog()` / `mapServerDeleteLog()` / `deduplicateDeleteLogs()` from `deleteLogs.ts`, never generate timestamp-based log ids. Delete handlers must clear their pending-delete UI state before awaiting the deletion, so a double-click cannot delete twice.
